...
function concatenateStrings(value1, value2) {
  const result = [];
  return result.concat(value1, value2).join('');
}
...
function isPalindrome(str) {
  ...
  const newStr = str.replace(letters, '').toLocaleLowerCase();
  const reverseStr = newStr.split('').reverse().join('').toLocaleLowerCase();
...
function findLongestWord(sentence) {
  ...
  const maxLength = Math.max(...lengthsWords);
  const indexWord = lengthsWords.indexOf(maxLength);
...
function encodeToRot13(str) {
  const algorithm = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const rot13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let result = '';
  const letters = /[a-zA-Z]/;

  for (let i = 0; i < str.length; i += 1) {
    const index = algorithm.indexOf(str[i]);
    result += str[i].replace(letters, rot13[index]);
  }
...
function getCardId(value) {
  ...
  const suits = '♣♦♥♠';
  const indexNumCard = numCard.indexOf(value.slice(0, value.length - 1));
  const indexSuits = suits.indexOf(value.at(-1));

  return indexSuits * 13 + indexNumCard;
}
